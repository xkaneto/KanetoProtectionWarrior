...
namespace AimsharpWow.Modules
{
    ...
    default:
        ...
        public override void Initialize()
        {
            ...
            CustomFunctions.Add("GetLatency", "return GetNetStats();");
            CustomFunctions.Add("GetSpellQueueWindow", "return tonumber(GetCVar('SpellQueueWindow'));");
        }
        ...
        public override bool CombatTick() 
        {
            ...
            Aimsharp.PrintMessage("Der sqw ist momentan: " + sqw, Color.Yellow);
            ...
            if (Fighting) 
            {
                ...
                if (!NoFocus && FocusHealth <= GetSlider("Auto Intervene @ Focus HP%") && Aimsharp.CanCast(Intervene_SpellName(), "player"))
                {
                ...
                if (!InterruptsOff)
                {
                    ...
                    if (TalentStormbolt)
                    {
                        if (!Aimsharp.CanCast(Pummel_SpellName())&& Aimsharp.CanCast(StormBolt_SpellName()) && CastingRemaining < KickValue && (IsChannelingTar || IsInterruptable || !IsInterruptable))
                        {
                ...
                {
                    if (IsChannelingTar && CastingElapsed > 500)
                    {
                ...
                {                    
                    ...
                    if (EnemiesInMelee >= 2 && Aimsharp.CanCast(Shockwave_SpellName(), "player") && CastingRemaining < KickValue && (IsChannelingTar || IsInterruptable || !IsInterruptable)) 
                    {
                        Aimsharp.Cast(Shockwave_SpellName());
                        return true;
                    }
                ...
                if (Aimsharp.CanCast(Shockwave_SpellName(), "player") && CastingRemaining < KickValue && (IsChannelingTar || IsInterruptable || !IsInterruptable)) 
                {
                       Aimsharp.Cast(Shockwave_SpellName());
                       return true;
                }
